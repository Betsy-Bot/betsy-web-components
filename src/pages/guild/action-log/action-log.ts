...
export class ActionLog implements IRouteViewModel {
    ...
    async attached() {
        ...
        this.featureActive = this.guild.activeFeatures.includes(
            this.discordService.AUDIT_LOG
        );
        for (const feature of this.guild.activeAuditLogFeatures) {
            const index = this.actionLogItems.findIndex(
                (x) => x.value === feature
            );
    ...
    async toggleFeature() {
        if (this.featureActive) {
            ...
            this.guild.activeFeatures = this.guild.activeFeatures.filter(
                (x) => x !== this.discordService.AUDIT_LOG
            );
            await this.discordService.setActiveFeaturesForDiscord(
                this.guildId,
                this.guild.activeFeatures
    ...
    async toggleActionFeature(feature) {
        if (!this.isFeatureOn(feature.value)) {
            ...
            this.guild.activeAuditLogFeatures =
                this.guild.activeAuditLogFeatures.filter(
                    (x) => x !== feature.value
                );
        }
        await this.discordService.setActiveAuditFeaturesForDiscord(
            this.guildId,
            this.guild.activeAuditLogFeatures
        );
        toast(
            this.editActive
                ? `Toggled logging on for ${feature.label}`
                : `Toggled logging off for ${feature.label}`
        );
    }
